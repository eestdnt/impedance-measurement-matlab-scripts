...
subplot(2, 1, 1);
stairs(tv, measured_excitation_signal), grid("on"), ylabel("Current (A)"), title("Excitation");
hold("on"), xline(1/Fs*(length(measured_excitation_signal) - N*mult*P), "Color", "red"), hold("off");
subplot(2, 1, 2);
plot(tv, measured_response_signal), grid("on"), ylabel("Voltage (V)"), title("Response");
hold("on"), xline(1/Fs*(length(measured_excitation_signal) - N*mult*P), "Color", "red"), hold("off");
xlabel("Time (s)");
sgtitle("Raw signals");
...
subplot(2, 1, 1);
stairs(tv, x), grid("on"), ylabel("Current (A)"), title("Excitation");
subplot(2, 1, 2);
stairs(tv, y), grid("on"), ylabel("Voltage (V)"), title("Response");
xlabel("Time (s)");
...
xlim([f1, f_bw]), ylabel("Magnitude (db)"), grid("on");
subplot(2, 1, 2);
